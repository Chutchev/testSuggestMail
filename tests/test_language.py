...
def test_language(driver: WebDriver):
    ...
    BaseSteps.send_keys(driver, page, "поле", "Поле поиска", text)
    # 4. Нашли все подсказки и сохранили их.
    time.sleep(1)
    translit_symbols_elements = BaseSteps.find(driver, page, "опция", "")
    translit_symbols_text = [element.text for element in translit_symbols_elements if "привет" in element.text and "hello" in element.text]
    assert len(translit_symbols_text) == len(translit_symbols_elements), "Учитываются не оба языка"